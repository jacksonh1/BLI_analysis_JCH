...
class BLI_data2:
    ...
    def __init__(self, datafile, sample_key):
        # Store data in a DataFrame
        df = pd.read_csv(datafile, sep='\t')
        # remove the last column
        df = df.iloc[:,:-1]
        ...
        for i in df.columns:
            ...
            wells.append(i)
        self.sample_wells = wells
        self.sample_key = pd.read_csv(sample_key)
        self.df = df

    def set_wells_from_samplekey(self, tip, titrant):
        # use the sample key to get the wells for this set of curves
        # if input arguments arent lists make them lists
        tip = [tip] if isinstance(tip, str) else tip
        titrant = [titrant] if isinstance(titrant, str) else titrant
    ...
    def binding_signal_preview(self, subtracted_data=False):
        fig, ax = plt.subplots(figsize = [15,7])
        if subtracted_data:
            dat = self.baseline_subtracted_data
        else:
